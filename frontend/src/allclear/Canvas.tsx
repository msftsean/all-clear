  ...
  return (
    <Card title="map" testid="map-card">
      <div className="relative overflow-hidden rounded-[16px] border border-nline bg-night shadow-dark-glass">
        ...
        >
          <rect width="320" height="150" fill="#F7EDD4" />
          {/* faint reference grid */}
          {Array.from({ length: 7 }).map((_, i) => (
            <line key={`h${i}`} x1="0" y1={i * 22 + 8} x2="320" y2={i * 22 + 8} stroke="#D7B262" strokeWidth="1" />
          ))}
          {Array.from({ length: 11 }).map((_, i) => (
            <line key={`v${i}`} x1={i * 30 + 6} y1="0" x2={i * 30 + 6} y2="150" stroke="#D7B262" strokeWidth="1" />
          ))}
  ...
  return (
    <Card title="ClearBoard · pins merge on dedup" testid="hero-map-card" span>
      <div className="relative overflow-hidden rounded-[16px] border border-nline bg-night shadow-dark-glass">
        ...
        >
          <rect width="320" height="170" fill="#F7EDD4" />
          {Array.from({ length: 8 }).map((_, i) => (
            <line key={`h${i}`} x1="0" y1={i * 22 + 8} x2="320" y2={i * 22 + 8} stroke="#D7B262" strokeWidth="1" />
          ))}
          {Array.from({ length: 11 }).map((_, i) => (
            <line key={`v${i}`} x1={i * 30 + 6} y1="0" x2={i * 30 + 6} y2="170" stroke="#D7B262" strokeWidth="1" />
          ))}

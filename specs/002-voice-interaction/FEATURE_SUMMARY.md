...
- ✅ `AzureRealtimeService` — production implementation (httpx, `gpt-realtime`)
...
| Layer | Count | Details |
|-------|-------|---------|
| **Backend unit tests** | **435 / 435 ✅** | Config fields, Pydantic models, MockRealtimeService, API endpoints, PII filter, existing pipeline tests, plus 97 GPT-4.1 model evals |
...
- **435 backend tests pass** — voice models, services, endpoints, PII filter, existing pipeline tests, plus 97 GPT-4.1 model evals (intent, PII, sentiment, entities, urgency, e2e) running via `DefaultAzureCredential`

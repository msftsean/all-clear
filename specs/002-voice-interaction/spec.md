...
**Input**: User description: "Add real-time voice conversation using Azure OpenAI GPT Realtime API via WebRTC, enabling students to speak naturally with the support agent instead of typing"
...
- **VFR-002**: System MUST use the `gpt-realtime` model via WebRTC transport (browser RTCPeerConnection direct to Azure OpenAI endpoint)
...
**Infrastructure**
- **VFR-023**: The Azure infrastructure (Bicep) MUST include a `gpt-realtime` model deployment in the Azure OpenAI resource
...
- Azure OpenAI `gpt-realtime` model is available for deployment in the same region as the existing `gpt-4.1` deployment
...
- Azure OpenAI `gpt-realtime` model deployment — **required for production voice**

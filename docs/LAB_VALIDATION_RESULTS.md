...
2. **retrieve_agent.py** - Changed default model from `gpt-4` to `gpt-4.1`:
   ```python
...
**250 lines defining:**
- Azure OpenAI (gpt-4.1 deployment, S0 SKU)
...
| DeploymentNotFound for gpt-4 | Changed default model to gpt-4.1 |
...
|-------|-----------------|---------|
| GPT-4.1 | `gpt-4.1` | Chat completions, intent classification, response generation |
...
| `labs/04-build-rag-pipeline/solution/retrieve_agent.py` | Changed default model to gpt-4.1 |
...
1. **Create Azure Deployments First**: Before starting Lab 04, ensure both `gpt-4.1` and `text-embedding-ada-002` are deployed in Azure AI Foundry
...
2. **Document Model Requirements**: Clearly state that both `gpt-4.1` and `text-embedding-ada-002` deployments are required

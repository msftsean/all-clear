...
const runOfShow = [
  ...
  {
    time: "1:08–1:22",
    block: "The pattern — three agents",
    lead: "Sean",
    min: 14,
  },
...
const lanes = [
  {
    ...
    tradeoff:
      "You’re proving the brain — the decisions — not the live plumbing.",
  },
  {
    name: "Self-Serve",
    ...
    plain:
      "azd up into your own sponsorship resource group — a real artifact you keep.",
    tradeoff:
      "A few minutes of provisioning up front — but it’s yours, and it goes home to your campus on Monday.",
  },
  {
    name: "Shared",
    ...
    tradeoff:
      "You share it, so you can’t take that exact instance home — that’s what Self-Serve is for.",
  },
];
...
const slide10IfAsked = [
  [
    '"Which lane should my team use?"',
    "Start in Mock — everyone — to confirm your logic. Then run azd up for the live version. Shared is only if you want to skip provisioning entirely.",
  ],
  [
    '"Why mock first instead of just deploying?"',
    "Because mock is deterministic and free. You get your routing and safety right with zero tokens and zero cloud variables, then deploy with confidence — live should match the baseline.",
  ],
  [
    '"What does azd up actually stand up?"',
    "Azure OpenAI with the GPT-4.1, realtime, and embedding models, AI Search, and Container Apps, plus the backend and frontend — in one command, into your own resource group.",
  ],
  [
    '"What if azd up fails?" (only if pressed)',
    "Almost always one of three things — model quota, resource-group permissions, or an unregistered provider. Your coach has the runbook with the exact fix for each. You won’t lose momentum — your mock baseline is still running.",
  ],
];
...
const slide21IfAsked = [
  [
    '"Can two teams take the same card?"',
    "Sure — you’ll be amazed how different two agents are for the same message. But try to spread across the doors so we see all six patterns at demo time.",
  ],
  [
    '"Is Card D off-limits since it was the live demo?"',
    "Take it if you want — seeing it run doesn’t mean it’s easy. D has the richest logic on the board: two parallel actions from one message.",
  ],
  [
    '"Which is the easiest to start with?"',
    "E, the warmup — simplest routing plus the interfaith welcome. Get a clean win there, then reach for B or D for the harder builds.",
  ],
  [
    '"How ‘done’ does it need to be?"',
    "Working beats perfect. If your routing is right and your escalation actually fires on stage — observed, not described — you’ve nailed the rubric.",
  ],
];
...
function SectionTitle({
  ...
  return (
    <div className="mb-4 mt-12 first:mt-0">
      <h3 className="text-2xl font-semibold tracking-tight text-dark-text">
        {children}
      </h3>
...
export default function RunOfShow() {
  return (
    <div
      role="tabpanel"
      id="run-of-show-panel"
      aria-labelledby="run-of-show-tab"
    >
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-xl text-ink/70 mb-6">
          Presenter quick-reference — for when you get distracted. AJCU
          Pre-Conference Workshop · Fordham Lincoln Center, LAW 3-09 · Mon Jun
          1, 2026 · 1:00–4:00 PM.
        </p>

        <div className="callout border-brand/30 bg-brand/5 mb-10">
          <p className="text-dark-text leading-relaxed">
            <strong>azd-first.</strong> Provision once, then build. Everyone
            starts in
            <strong> Mock</strong> (offline, deterministic, zero credentials) to
            get the decisions right, then graduates to{" "}
            <strong>Self-Serve</strong> with one command —{" "}
            <code className="px-1 bg-surface-muted rounded">azd up</code> — into
            their own resource group. The bulk of the afternoon is two build
            sprints and team demos, not configuration.
          </p>
        </div>
        ...
        <div className="callout border-iu-crimson/40 bg-iu-crimson/5 mb-10">
          <p className="text-sm font-bold uppercase tracking-wider text-iu-crimson mb-2">
            🎤 Live Demo · Card D — Phishing Storm{" "}
            <span className="font-medium normal-case tracking-normal text-ink/60">
              (1:22–1:32 · Jake drives)
            </span>
          </p>
          <p className="text-dark-text leading-relaxed mb-3">
            Open the live app and type this <strong>exact</strong> message into
            the chat:
          </p>
          <blockquote className="bg-white/70 px-4 py-3 my-2 text-dark-text rounded-lg font-mono text-sm border border-iu-crimson/20">
            My password isn&apos;t working and I just got an email saying my
            account is locked. I clicked the link.
          </blockquote>
          <p className="text-dark-text leading-relaxed mt-3">
            <strong>What the room sees</strong> (verified live, deterministic):
            routes to
            <strong> IT</strong> → <strong>creates a ticket</strong> → returns
            <strong> 3 knowledge articles</strong>. The teaching point: one
            message, the agent acts on the second signal —{" "}
            <em>&ldquo;I clicked the link&rdquo;</em> — not just the obvious
            password reset.
          </p>
          <p className="text-dark-text leading-relaxed mt-3">
            <strong>Backup safety beat</strong> (optional, shows the override):
            type
            <em> &ldquo;I want to kill myself&rdquo;</em> → escalates to a human
            + the
            <strong> 988</strong> line, and never answers a crisis with
          ...
          <div className="mt-4 grid sm:grid-cols-2 gap-3 text-sm">
            ...
            <div className="bg-white/60 rounded-lg p-3">
              <p className="font-semibold text-iu-crimson mb-1">
                ⚠️ Do NOT type live
              </p>
              <p className="text-ink/75 leading-relaxed">
                The <strong>Mass of the Holy Spirit</strong> (Card E) or Spanish
                lines — the live shared router sends them to IT. Demo{" "}
                <strong>Card D</strong> only.
              </p>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {runOfShow.map((r) => (
                <tr key={r.time} className="border-t border-border">
                  <td className="py-2.5 px-4 font-mono text-ink/70 whitespace-nowrap">
                    {r.time}
                  </td>
                  <td className="py-2.5 px-4 text-dark-text font-medium">
                    {r.block}
                  </td>
                  <td className="py-2.5 px-4 text-ink/70">{r.lead}</td>
                  <td className="py-2.5 px-4 text-right text-ink/70">
                    {r.min}
                  </td>
                </tr>
              ))}
          ...
          {lanes.map((lane) => (
            ...
            >
              ...
              <p className="text-sm text-ink/75 mt-2">
                <strong className="text-dark-text">What it is:</strong>{" "}
                {lane.plain}
              </p>
              <p className="text-sm text-ink/75">
                <strong className="text-dark-text">Trade-off:</strong>{" "}
                {lane.tradeoff}
              </p>
            </div>
          ))}
        ...
        <div className="callout border-iu-crimson/30 bg-iu-crimson/5 mt-4">
          <p className="text-dark-text leading-relaxed text-sm">
            <strong>⚠️ Confirm before stage:</strong> the{" "}
            <strong>Shared</strong> lane is the one detail the Student Runbook
            doesn’t document (it walks Mock → azd up). Check with Jake /
            Cloudforce at the top of the session: if a hosted shared backend is
            live for the room, keep the Shared beat; if not, present this as
        ...
        <p className="text-sm text-ink/70 mt-3">
          Then deliver each lane (Mock → Self-Serve → Shared) using the cards
          above — verbatim line, then “what it is,” then the trade-off.
        </p>
        ...
        <div className="mb-3">
          <p className="font-semibold text-dark-text">
            Frame the slide (say this to set it up)
          </p>
          <Verbatim>
            "Every card is one real student message — that’s the door. The agent
            behind it is the routing-and-escalation logic your team builds so
            that message reaches the right office, the right way. You’re not
            building six apps. You’re building one agent’s judgment, proven
            against the one scenario you pick. Six doors, and behind each one a
            different lesson in how an agent should decide."
          </Verbatim>
          <p className="font-semibold text-dark-text mt-3">
            Opening line (say this first)
          </p>
          <Verbatim>
            "Here’s where it becomes yours. Six real student messages — six
            doors. You already watched Card D run live a few minutes ago. Now
            your team picks one and builds the agent that answers it the way a
            Jesuit university should."
          </Verbatim>
        </div>
        ...
        <div className="mb-3 mt-4">
          <p className="font-semibold text-dark-text">
            Transition (say this last)
          </p>
          <Verbatim>
            "You just saw D run live, and E is your gentlest warmup if you’re
            new. So pick your door — A, B, C, E, or F, or take on D yourselves.
            Six doors, five teams, one working agent by 3:00. Pick your door,
            and build the agent behind it."
          </Verbatim>
        </div>

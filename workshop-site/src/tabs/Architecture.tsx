  ...
  return (
    <div role="tabpanel" id="architecture-panel" aria-labelledby="architecture-tab">
      <div className="max-w-5xl mx-auto">
        ...
        >
          {/* Student */}
          <circle cx="50" cy="150" r="30" fill="#0F6CBD" />
          <text x="50" y="155" textAnchor="middle" className="diagram-text" fill="white" fontSize="12">
            Student
          </text>

          {/* Arrow to QueryAgent */}
          <path d="M 85 150 L 130 150" stroke="#1B1B1F" strokeWidth="2" markerEnd="url(#arrowhead)" />

          {/* QueryAgent */}
          <rect x="135" y="110" width="120" height="80" fill="#F7F8FA" stroke="#0F6CBD" strokeWidth="2" rx="8" />
          <text x="195" y="140" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#1B1B1F">
            QueryAgent
          </text>
          ...
          <path d="M 260 150 L 305 150" stroke="#1B1B1F" strokeWidth="2" markerEnd="url(#arrowhead)" />

          {/* RouterAgent */}
          <rect x="310" y="110" width="120" height="80" fill="#F7F8FA" stroke="#0F6CBD" strokeWidth="2" rx="8" />
          <text x="370" y="140" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#1B1B1F">
            RouterAgent
          </text>
          ...
          <path d="M 435 150 L 480 150" stroke="#1B1B1F" strokeWidth="2" markerEnd="url(#arrowhead)" />

          {/* ActionAgent */}
          <rect x="485" y="110" width="120" height="80" fill="#F7F8FA" stroke="#0F6CBD" strokeWidth="2" rx="8" />
          <text x="545" y="140" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#1B1B1F">
            ActionAgent
          </text>
          ...
          {/* Arrow to Response */}
          <path d="M 610 150 L 655 150" stroke="#1B1B1F" strokeWidth="2" markerEnd="url(#arrowhead)" />

          {/* Response */}
          <rect x="660" y="125" width="100" height="50" fill="#0F6CBD" rx="8" />
        ...
        <div className="grid md:grid-cols-3 gap-6 my-12">
          {agents.map((agent, index) => (
            <div key={index} className="card">
              <div className="text-2xl font-bold text-brand mb-2">{agent.name}</div>
              <p className="text-sm font-semibold text-dark-text mb-2">{agent.role}</p>
              <p className="text-sm text-ink/70 mb-3">{agent.responsibility}</p>
              <div className="border-t border-border pt-3 mt-3">
                <p className="text-xs font-medium text-ink/60 mb-1">OUTPUT:</p>
                <p className="text-xs text-ink/75">{agent.output}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="card mt-8">
          ...
          <div className="space-y-3">
            {tools.map((tool, index) => (
              <div key={index} className="flex items-start gap-3 p-3 bg-surface-muted rounded">
                <span className="text-brand font-mono text-sm flex-shrink-0">
                  {index + 1}.
                </span>
                <div>
                  <code className="text-sm font-semibold text-dark-text">{tool.name}</code>
                  <p className="text-sm text-ink/70 mt-1">{tool.purpose}</p>
                </div>
              </div>
            ))}
          </div>

...
function SectionCard({ section }: { section: ScriptSection }) {
  ...
  return (
    <div
      className={`card ${style.border}`}
      role="region"
      aria-label={`${section.title} — ${section.timeStart} to ${section.timeEnd}`}
    >
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full text-left focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2 rounded"
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4 flex-1">
            ...
            <div className="flex-shrink-0 text-center min-w-[5rem]">
              ...
              <div className="text-xs text-ink/60">to {section.timeEnd}</div>
              <div className="mt-1 inline-block px-2 py-0.5 bg-surface-muted rounded text-xs font-medium text-ink/70">
            ...
            <div className="flex-1">
              ...
              {/* Show target teaser */}
              <p className="text-sm text-ink/60 mt-1">📺 {section.showTarget}</p>
            </div>
          </div>

          {/* Collapse toggle */}
          <div className="flex-shrink-0 mt-1 text-ink/40">
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <span className="text-xs text-ink/60 flex-shrink-0">{section.elapsedEnd}/45 min</span>
        </div>
      </button>
      ...
      {isOpen && (
        <div id={`section-${section.id}`} className="mt-5 space-y-4">
          {/* Talk track */}
          <div>
            <h4 className="text-sm font-semibold text-ink/60 uppercase tracking-wide mb-2">
              🗣️ Talk Track
            </h4>
            <div className="space-y-2">
              {section.talkTrack.map((line, i) => (
                <p key={i} className="text-ink/75 italic bg-surface-muted rounded-lg p-3 text-sm leading-relaxed border border-border">
                  {line}
                </p>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {section.actions.map((action, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2 text-sm bg-brand/5 rounded-lg px-3 py-2 border border-brand/20"
                >
                  <span className="flex-shrink-0">{action.icon}</span>
                  <span className="text-ink/80">{action.text}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Show target callout */}
          <div className="bg-surface-muted rounded-lg p-3 flex items-center gap-3 border border-border">
            <span className="text-lg">📺</span>
            <div>
              <span className="text-xs font-semibold text-ink/60 uppercase">What to show</span>
  ...
  return (
    <div role="tabpanel" id="presenter-script-panel" aria-labelledby="presenter-script-tab">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="card mb-8">
          <div className="flex flex-wrap items-center gap-6 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-ink/75">Narrative</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-green-500 inline-block" />
              <span className="text-sm text-ink/75">Live Demo</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-amber-500 inline-block" />
              <span className="text-sm text-ink/75">Exercise</span>
            </div>
          </div>
        ...
        <div className="mt-10 card bg-surface-muted">

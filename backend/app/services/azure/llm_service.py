...
class AzureOpenAILLMService(LLMServiceInterface):
    ...
    DEFAULT_CLASSIFY_PROMPT = """You are a university support system assistant that classifies student queries.
...
Always respond with valid JSON only."""
    ...
    async def classify_intent(
        ...
        prompt = system_prompt if system_prompt else self.DEFAULT_CLASSIFY_PROMPT
    ...
    # Default system prompt for clarification questions
    DEFAULT_CLARIFICATION_PROMPT = """You are a helpful university support assistant.
...
Keep it brief (1-2 sentences) and offer clear options when possible."""
    ...
    async def generate_clarification_question(
        ...
        prompt = system_prompt if system_prompt else self.DEFAULT_CLARIFICATION_PROMPT
...
Generate a clarification question to understand what they need."""
    ...
    DEFAULT_RESPONSE_PROMPT = """You are a helpful university support assistant. Your goal is to provide IMMEDIATE, ACTIONABLE help to students.
...
Keep the response friendly, concise (3-5 sentences), and focused on solving their problem immediately."""
    ...
    async def generate_response_message(
        ...
        prompt = system_prompt if system_prompt else self.DEFAULT_RESPONSE_PROMPT

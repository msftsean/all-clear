...
class Settings(BaseSettings):
    ...
    # ==========================================================================
    # All Clear: RouterExecutor severity SLA (minutes) and dedup threshold
    # (CONTEXT.md severity/SLA matrix; thresholds live in config, never in code)
    # ==========================================================================
    sla_sev1_minutes: int = Field(
        default=15, gt=0, description="SEV1 response SLA in minutes (life safety / total outage)"
    )
    sla_sev2_minutes: int = Field(
        default=60, gt=0, description="SEV2 response SLA in minutes (major, public-facing)"
    )
    sla_sev3_minutes: int = Field(
        default=240, gt=0, description="SEV3 response SLA in minutes (contained, single-party)"
    )
    sla_sev4_minutes: int = Field(
        default=1440, gt=0, description="SEV4 response SLA in minutes (next-business-day default)"
    )
    dedup_threshold: float = Field(
        default=0.83,
        ge=0.0,
        le=1.0,
        description="Cosine similarity at or above which a signal attaches to an open incident",
    )

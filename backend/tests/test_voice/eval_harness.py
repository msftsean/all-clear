...
class TestVoiceEvalHarness:
    ...
    async def test_eval_scenario(self, service, scenario):
        ...
        assert latency_ms < 5000, f"Tool {scenario['tool']} took {latency_ms:.2f}ms"
    ...
    @pytest.mark.asyncio
    async def test_tool_latency_p95_under_5000ms(self, service, capsys):
        """T070: P95 tool round-trip latency remains under 5000ms with per-tool summary."""
        timings: dict[str, list[float]] = defaultdict(list)
        scenarios = [
            ("analyze_and_route_query", {"query": "I forgot my password"}),
            ("check_ticket_status", {"ticket_id": "TKT-IT-20260313-0001"}),
            ("search_knowledge_base", {"query": "reset password"}),
            ("escalate_to_human", {"reason": "Student wants refund appeal"}),
        ]
        ...
        for tool_name, arguments in scenarios:
            for index in range(5):
                start = time.perf_counter()
                result = await service.execute_tool(
                    call_id=f"latency-{tool_name}-{index}",
                    tool_name=tool_name,
                    arguments=arguments,
                    session_id="eval-latency-session",
                )
                elapsed_ms = (time.perf_counter() - start) * 1000
                timings[tool_name].append(elapsed_ms)
                assert result.error is None

        all_timings = sorted(value for values in timings.values() for value in values)
        p95_index = max(0, int(len(all_timings) * 0.95) - 1)
        p95_ms = all_timings[p95_index]

        print("\nVoice tool latency summary")
        print("tool | count | avg_ms | max_ms")
        for tool_name, values in timings.items():
            avg_ms = sum(values) / len(values)
            print(f"{tool_name} | {len(values)} | {avg_ms:.2f} | {max(values):.2f}")

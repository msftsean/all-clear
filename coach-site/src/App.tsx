...
const navSections = sectionEntries.map((e) => ({
  id: e.meta.id,
  title: e.meta.title,
}));
...
function App() {
  ...
  const [heroCollapsed, setHeroCollapsed] = useState<boolean>(() =>
    typeof window !== "undefined"
      ? readHashSection() !== DEFAULT_SECTION_ID
      : false,
  );
  ...
  const active =
    sectionEntries.find((e) => e.meta.id === activeSectionId) ??
    sectionEntries[0];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-paper">
      ...
      <main
        ...
        tabIndex={-1}
        ref={mainRef}
        className="flex-1 max-w-5xl w-full mx-auto px-4 py-12"
      >
        ...
        <section aria-labelledby="intro-heading" className="mb-12" data-reveal>
          ...
          <div className="card-raised bg-accent-soft rounded-2xl">
            <div className="flex gap-4">
              <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-surface/80 border border-border flex items-center justify-center shadow-soft">
                <span aria-hidden="true" className="text-cofounder text-lg">
                  ◆
                </span>
              </div>
              <div className="min-w-0">
                <p className="text-ink/80 leading-relaxed">
                  This is a calm, scannable companion for{" "}
                  <strong>coaches</strong> running the 47 Doors AJCU hackathon.
                  It re-presents the existing coach guides — how to prepare the
                  room and tech, run the day, frame the mission, help blocked
                  participants, troubleshoot setup failures, and assess the
                  build sprint.
                </p>
                <p className="text-ink/80 leading-relaxed mt-3">
                  Use it on a laptop or phone, before and during the event. Jump
                  to any of the six sections below; <strong>Prepare</strong> is
                  the place to start the night before.
                </p>
              </div>
            </div>
                ...
                return (
                  <li key={e.meta.id}>
                    <button
                      ...
                      className="text-left w-full flex items-start gap-3 px-3 py-2.5 rounded-xl hover:bg-surface/70 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-cofounder focus-visible:ring-offset-2"
                    >
                      {Icon && (
                        <Icon
                          className="mt-0.5 flex-shrink-0 text-cofounder"
                          style={{ width: 18, height: 18 }}
                          aria-hidden="true"
                        />
                      )}
                      <span>
                        <span className="font-semibold text-cofounder">
                          {e.meta.title}
                        </span>
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            {(() => {
              ...
              return Icon ? (
                <div className="w-9 h-9 rounded-xl bg-accent-soft border border-border flex items-center justify-center flex-shrink-0">
                  <Icon
                    className="text-cofounder"
                    style={{ width: 18, height: 18 }}
                    aria-hidden="true"
                  />
                </div>

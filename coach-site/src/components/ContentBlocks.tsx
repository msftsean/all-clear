...
function BlockRenderer({ block }: { block: ContentBlock }) {
  switch (block.kind) {
    ...
    case "lane":
      return (
        <div className="card bg-accent-soft border-border">
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
            <span
              className="h-2.5 w-2.5 rounded-full bg-cofounder"
              aria-hidden="true"
            />
            <h3 className="font-semibold text-ink">{block.name}</h3>
            <span className="text-sm font-medium text-cofounder">
              {block.outcome}
            </span>
          </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <thead>
            <tr className="bg-accent-soft">
              <th
                scope="col"
                className="px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-ink/70 whitespace-nowrap"
              >
                Time
              </th>
              <th
                scope="col"
                className="px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-ink/70"
              >
                Block
              </th>
              <th
              ...
              <th
                scope="col"
                className="px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-ink/70 text-right"
              >
                Min
              </th>
            </tr>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr
                key={i}
                className="border-t border-border hover:bg-accent-soft/55 transition-colors"
              >
                ...
                <td className="px-4 py-3 text-right text-ink/55 tabular-nums">
                  {r.min}
                </td>
              </tr>
            ))}
          ...
          <tfoot>
            <tr className="border-t border-border bg-paper">
              <td
                className="px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-ink/60"
                colSpan={3}
              >
                Total
              </td>
              <td className="px-4 py-2.5 text-right font-semibold text-ink tabular-nums">
                {total}
              </td>
            </tr>
          </tfoot>
